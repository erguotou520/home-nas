...
## 8. 当前边界与后续开发建议
1. 媒体流、缩略图、歌词、媒体元数据接口需要在 Axum + SQLx 基线稳定后继续补齐。
2. 建议引入 SQLx migration 机制替代启动时建表 SQL。
3. 建议增加集成测试：认证流程、分享过期与访问次数、文件路径安全校验。
4. 建议收紧 CORS 配置并引入刷新令牌机制。
